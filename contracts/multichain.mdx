...
| Cronos Testnet | 338 | Custom (api bundler) | x402 + Facilitator |
...
sequenceDiagram
    ...
    participant API as API Bundler
    ...
    Frontend->>API: POST /api/aa/prepare
    API->>Factory: getAddress(admin, "")
    Factory-->>API: Predicted account address
    API-->>Frontend: UserOp + hash to sign
    
    User->>Frontend: Sign UserOpHash
    Frontend->>API: POST /api/aa/submit
    API->>Paymaster: Sign sponsorship
    API->>EntryPoint: handleOps([userOp])
    EntryPoint->>Factory: Deploy if needed
    EntryPoint-->>API: Success
...
| Component | Location |
|:----------|:---------|
| UserOp construction | `backend/api/shared/aa/userop.ts` |
| Paymaster signing | `backend/api/shared/aa/paymaster.ts` |
| Frontend AA client | `app/src/lib/cronos/aa.ts` |
| Cronos Facilitator | `backend/api/shared/config/cronos.ts` |
