...
Every agent in the same swarm shares the same `rootComposeRunId`, so the coordinator and descendants can read the same bus. Each write records `writtenBy`, `ts`, and a monotonic `version`.
