...
Use the AI SDK OpenAI-compatible provider when you want AI SDK ergonomics with Compose billing and model routing.
...
`gpt-5.5` and `openai/gpt-5.5` are different public IDs because they resolve to different provider rows.
...
If your backend needs receipts, call native `/v1/*` directly and read `X-Compose-Receipt`.
