...
Compose does not execute your tools. Your client or runtime owns permissions, sandboxing, side effects, and the next request.
...
The gateway normalizes tool requests, but providers still differ:
...
OpenCode and similar IDEs send large tool payloads. The external gateway keeps those fields intact and passes AI SDK wrapper fields through canonical normalization where the core understands them.
