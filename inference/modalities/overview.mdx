...
Specialized outputs such as detections, segmentation, keypoints, and classification appear in catalog metadata for providers such as Roboflow. External OpenAI-compatible configs filter those labels when the target client only accepts a smaller modality enum.
...
Use these endpoints when you need model discovery by task instead of by provider name.
