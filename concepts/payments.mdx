...
| **Session Bypass** | `x-session-*` headers | Active web sessions | below 100ms |
...
```bash
curl -X POST https://api.compose.market/agent/{id}/chat \
...
```
x-runtime-internal: {RUNTIME_INTERNAL_SECRET}
```
