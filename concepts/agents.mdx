...
| Service | URL | Purpose |
|---------|-----|---------|
| api/ | `api.compose.market` | Discovery, models, inference, agent registration |
| runtime | `runtime.compose.market` | Agent chat,  Workflows|
...
```
POST https://api.compose.market/agent/{id}/chat     # Chat
POST https://api.compose.market/agent/{id}/stream   # Streaming
POST https://api.compose.market/agent/{id}/multimodal  # Image/audio
```

## Registration

```
POST https://api.compose.market/agent/register
```
...
Chat endpoints require x402 payment via the `x-payment` header. For nested Workflow calls, the `x-runtime-internal` header bypasses payment.
